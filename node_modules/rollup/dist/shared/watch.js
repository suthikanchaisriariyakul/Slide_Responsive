...
micromatch.contains = (str, pattern, options) => {
  if (typeof str !== 'string') {
    throw new TypeError(`Expected a string: "${_util_commonjsExternal.require$$1.inspect(str)}"`);
  }
...
micromatch.parse = (patterns, options) => {
  let res = [];
  for (let pattern of [].concat(patterns || [])) {
    for (let str of index.braces(String(pattern), options)) {
...
micromatch.braces = (pattern, options) => {
  ...
  return index.braces(pattern, options);
};
